#!/bin/bash
# Tests for the Kyon Claude Code plugin hook scripts.
#
...
unset KYON_CLI_AGENT_PROTOCOL_VERSION
PAYLOAD=$(build_payload '{"session_id":"s1","cwd":"/tmp"}' "stop")
assert_json_field "defaults to v1 when env var absent" "$PAYLOAD" ".v" "1"

# Kyon declares v1 → use 1
export KYON_CLI_AGENT_PROTOCOL_VERSION=1
PAYLOAD=$(build_payload '{"session_id":"s1","cwd":"/tmp"}' "stop")
assert_json_field "v1 when kyon declares 1" "$PAYLOAD" ".v" "1"

# Kyon declares a higher version than the plugin knows → capped to plugin current
export KYON_CLI_AGENT_PROTOCOL_VERSION=99
PAYLOAD=$(build_payload '{"session_id":"s1","cwd":"/tmp"}' "stop")
assert_json_field "capped to plugin current when kyon is ahead" "$PAYLOAD" ".v" "1"

# Kyon declares a lower version than the plugin knows → use kyon's version
...
export KYON_CLI_AGENT_PROTOCOL_VERSION=3
PAYLOAD=$(build_payload '{"session_id":"s1","cwd":"/tmp"}' "stop")
assert_json_field "uses kyon version when plugin is ahead" "$PAYLOAD" ".v" "3"
...
# Legacy Kyon (TERM_PROGRAM=KyonTerminal, no protocol version)
OUTPUT=$(TERM_PROGRAM=KyonTerminal bash "$HOOK_DIR/on-session-start.sh" < /dev/null 2>/dev/null)
SYS_MSG=$(echo "$OUTPUT" | jq -r '.systemMessage // empty' 2>/dev/null)
assert_eq "legacy Kyon shows active message" \
    "🔔 Kyon plugin active. You'll receive native Kyon notifications when tasks complete or input is needed." \
    "$SYS_MSG"
