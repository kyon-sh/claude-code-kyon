...
# Shows welcome message, Kyon detection status, and emits plugin version
...
if ! command -v jq &>/dev/null; then
    cat << 'EOF'
{
  "systemMessage": "🚨 Kyon notifications require jq! Install it with your system package manager (e.g. brew install jq, apt install jq) 🚨"
}
EOF
    exit 0
fi
...
# export it now so the rest of this hook (kyon-notify.sh below) can use it.
...
# Emit structured notification with plugin version so Kyon can track it
